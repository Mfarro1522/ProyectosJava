...
private boolean puedeAgregarOperador(){
    String txt = txtCalculadora.getText();
    if(txt.isEmpty()) {
        return false;
    }
    char ultimo = txt.charAt(txt.length()-1);
    if (esOperador(ultimo)){
        return true;
    }
    return !esOperador(ultimo);
    
}

private boolean puedeAgregarOperador(char parentesis){
    String txt = txtCalculadora.getText();
    ...
    if (parentesis == '(') {
        return esOperador(ultimo) || ultimo == '(';
    } 
    else if (parentesis == ')') {
        return !esOperador(ultimo) && ultimo != '(' && Character.isDigit(ultimo) || ultimo == ')';
    }
